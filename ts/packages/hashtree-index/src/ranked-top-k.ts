...
export const RANKED_TOP_K_FORMAT = 'hashtree/ranked-top-k@1' as const;
...
export async function buildRankedTopK(
  btree: BTree,
  tree: HashTree,
  entriesByTerm: ReadonlyMap<string, readonly RankedTopKBuildEntry[]>,
  termStatistics: ReadonlyMap<string, RankedTermStats>,
  segment: RankedSearchSegmentManifest,
): Promise<{ manifest: RankedTopKManifest; roots: CID | null }> {
  const roots: Array<[string, CID]> = [];
  let blockCount = 0;
  ...
  for (const term of [...entriesByTerm.keys()].sort(compareStrings)) {
    ...
    if (!statistics) throw new Error(`Missing ranked top-k term statistics: ${term}`);
    ...
    if (sourceEntries.length < RANKED_TOP_K_MIN_DOCUMENT_FREQUENCY) continue;
    const entries = sourceEntries.map((entry) => ({
      ...
      right.score - left.score || compareStrings(left.entry.id, right.entry.id))
      .map(({ entry }) => entry);
    ...
    while (level.length > 1) {
      const parents: BuiltNode[] = [];
      for (let offset = 0; offset < level.length; offset += RANKED_TOP_K_FANOUT) {
        parents.push(await writeInternal(
          tree,
          level.slice(offset, offset + RANKED_TOP_K_FANOUT),
        ));
      }
      level = parents;
    }
    if (level.length !== 1) throw new Error(`Missing ranked top-k postings: ${term}`);
    roots.push([term, level[0].cid]);
  }

  const postingCount = roots.reduce(
    (count, [term]) => count + (termStatistics.get(term)?.documentFrequency ?? 0),
    0,
  );
...
export async function readRankedTopKNode(tree: HashTree, root: CID): Promise<RankedTopKNode> {
  ...
  if (!bytes) throw new Error('Unreadable ranked top-k node metadata');
...
export function encodeRankedTopKManifest(manifest: RankedTopKManifest): string {
  return JSON.stringify(manifest);
}

export function decodeRankedTopKManifest(raw: string): RankedTopKManifest {
  ...
  const manifest = {
    ...
    minimumDocumentFrequency: positiveInteger(
      value.minimumDocumentFrequency,
      'ranked top-k minimumDocumentFrequency',
    ),
...
function summarize(entries: readonly RankedTopKBuildEntry[]): RankedTopKSummary {
  const fields = emptyRecord<RankedTopKFieldBound>();
...
function mergeSummaries(summaries: readonly RankedTopKSummary[]): RankedTopKSummary {
  const fields = emptyRecord<RankedTopKFieldBound>();
  let count = 0;
  let minId: string | undefined;
  for (const summary of summaries) {
    count += summary.count;
    if (minId === undefined || summary.minId < minId) minId = summary.minId;
  ...
  if (minId === undefined) throw new Error('Missing ranked top-k summary id');
  return { count, minId, fields: sortedRecord(fields) };
}

async function writeLeaf(
  tree: HashTree,
  entries: readonly RankedTopKBuildEntry[],
): Promise<BuiltNode> {
...
async function writeInternal(tree: HashTree, children: readonly BuiltNode[]): Promise<BuiltNode> {
  ...
  const links = children.map((child, index): TreeEntry => ({
...
function decodeNode(raw: string): WireNode {
...
function decodeChildren(value: unknown): WireNode['children'] {
  if (!Array.isArray(value) || value.length === 0) {
...
function decodeSummary(value: unknown, label: string): RankedTopKSummary {
  ...
  for (const fieldName of Object.keys(rawFields).sort(compareStrings)) {
    if (!fieldName) throw new Error(`Invalid ${label} field name`);
  ...
  if (Object.keys(fields).length === 0) throw new Error(`Invalid ${label} fields`);
