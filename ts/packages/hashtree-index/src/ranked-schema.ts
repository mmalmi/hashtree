...
export const RANKED_SEARCH_SEGMENT_FORMAT = 'hashtree/ranked-search-segment@1' as const;
...
export function normalizeRankedBuildOptions(
  options: RankedSearchBuildOptions,
): NormalizedRankedBuildOptions {
  const fields = Object.entries(options.fields)
    .sort(([left], [right]) => compareStrings(left, right))
    .map(([name, field]) => {
      if (!name) throw new Error('Ranked search field names must not be empty');
      return {
        name,
        boost: positiveNumber(field.boost ?? 1, `fields.${name}.boost`),
        lengthNormalization: boundedNumber(
          field.lengthNormalization ?? 0.75,
          0,
          1,
          `fields.${name}.lengthNormalization`,
        ),
      };
    });
  if (fields.length === 0) {
...
export function encodePosting(posting: RankedPosting): string {
  return JSON.stringify(posting);
}

export function decodePosting(raw: string): RankedPosting {
  const value = jsonObject(raw, 'posting');
  const rawFields = objectValue(value.fields, 'posting.fields');
  const fields = emptyRecord<RankedPosting['fields'][string]>();
  if (Object.keys(rawFields).length === 0) throw new Error('Invalid empty posting.fields');
  ...
  for (const fieldName of Object.keys(rawFields).sort(compareStrings)) {
    ...
    const positions = field.positions.map((position, index) =>
      nonNegativeInteger(position, `posting.fields.${fieldName}.positions[${index}]`));
    if (positions.length !== frequency || positions.some((position, index) =>
...
export function encodeDocumentStats(stats: RankedDocumentStats): string {
  return JSON.stringify(stats);
}

export function decodeDocumentStats(raw: string): RankedDocumentStats {
  ...
  for (const fieldName of Object.keys(rawLengths).sort(compareStrings)) {
    lengths[fieldName] = nonNegativeInteger(
      rawLengths[fieldName],
      `document stats.lengths.${fieldName}`,
    );
  }
  return { lengths };
}

export function encodeTermStats(stats: RankedTermStats): string {
  return JSON.stringify(stats);
}

export function decodeTermStats(raw: string): RankedTermStats {
  const value = jsonObject(raw, 'term stats');
  const documentFrequency = positiveInteger(
    value.documentFrequency,
    'term stats.documentFrequency',
  );
  ...
  const fieldSets = value.fieldSets.map((rawFieldSet, index) => {
    ...
    const fields = [...fieldSet.fields] as string[];
    ...
    return {
      fields,
      documentFrequency: positiveInteger(
        fieldSet.documentFrequency,
        `term stats.fieldSets[${index}].documentFrequency`,
      ),
    };
  });
  ...
  const fieldSetFrequency = fieldSets.reduce(
    (total, fieldSet) => total + fieldSet.documentFrequency,
    0,
  );
...
export function decodeRankedManifest(raw: string): RankedSearchSegmentManifest {
  ...
  const fields = value.fields.map((rawField, index) =>
    decodeManifestField(rawField, index));
  if (new Set(fields.map((field) => field.name)).size !== fields.length) {
    throw new Error('Duplicate ranked search manifest field');
  }

  const manifest: RankedSearchSegmentManifest = {
    ...
    storedValueCount: nonNegativeInteger(
      value.storedValueCount,
      'manifest.storedValueCount',
    ),
...
function decodeManifestField(raw: unknown, index: number): RankedSearchFieldManifest {
  ...
  return {
    ...
    lengthNormalization: boundedNumber(
      ...
      0,
      1,
      `manifest.fields[${index}].lengthNormalization`,
    ),
    totalLength: nonNegativeInteger(field.totalLength, `manifest.fields[${index}].totalLength`),
    populatedDocumentCount: nonNegativeInteger(
      field.populatedDocumentCount,
      `manifest.fields[${index}].populatedDocumentCount`,
    ),
  };
}
...
function jsonObject(raw: string, label: string): Record<string, unknown> {
  try {
    ...
    if (error instanceof SyntaxError) throw new Error(`Invalid ${label} JSON`);
    throw error;
  }
}
...
function boundedNumber(value: unknown, min: number, max: number, label: string): number {
  const number = nonNegativeNumber(value, label);
  if (number < min || number > max) throw new Error(`Invalid ${label}`);
  return number;
}
