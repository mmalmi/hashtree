...
export async function buildRankedSegment(btree, tree, documents, options) {
    ...
    const sortedDocuments = [...documents].sort((left, right) => compareStrings(left.id, right.id));
    ...
    for (const document of sortedDocuments) {
        ...
        const documentPostings = new Map();
        for (const field of config.fields) {
            const tokenized = tokenizeRankedField(document.fields[field.name], config.maxTokensPerField);
...
function addFieldPostings(postings, fieldName, occurrences) {
...
function buildManifest(config, documentCount, termCount, postingCount, storedValueCount, totals, populated) {
    return {
...
function validateDocuments(documents) {
    ...
    for (const document of documents) {
        if (!document.id)
            throw new Error('Ranked search document ids must not be empty');
...
//# sourceMappingURL=ranked-build.js.map