...
| HT-MESH-001: HTL never increases and forwarding halts at zero | relayless mesh design + Freenet-style HTL | `rust/crates/hashtree-network/src/types.rs`, `ts/packages/hashtree-mesh/src/protocol.ts` | `formal_mesh_props.rs`, `core.test.ts` | `mesh-formal`, `ts-mesh-formal` | Verified |
| HT-MESH-002: forwarding predicate equivalence holds (`should_forward(htl) == htl > 0`) | relayless mesh design | `rust/crates/hashtree-network/src/types.rs`, `ts/packages/hashtree-mesh/src/protocol.ts` | `formal_mesh_props.rs`, `core.test.ts` | `mesh-formal`, `ts-mesh-formal` | Verified |
| HT-MESH-003: selector fairness/backoff ordering constraints hold | Freenet-inspired policy | `rust/crates/hashtree-network/src/peer_selector.rs`, `ts/packages/hashtree-mesh/src/peerSelector.ts` | `formal_mesh_props.rs`, `core.test.ts` | `mesh-formal`, `ts-mesh-formal` | Verified |
| HT-MESH-004: mesh frames reject invalid protocol/version/IDs/HTL and non-25050 events | relayless mesh frame spec (`htree.nostr.mesh.v1`) | `rust/crates/hashtree-network/src/types.rs`, `ts/packages/hashtree-mesh/src/types.ts` | `test_mesh_frame_validation_*`, `core.test.ts` | `mesh-formal`, `ts-mesh-formal` | Verified |
| HT-MESH-005: mesh seen-set dedupe rejects duplicate frame/event IDs and evicts oldest by cap | relayless mesh anti-loop requirement | `rust/crates/hashtree-network/src/types.rs` | `test_timed_seen_set_dedupe_and_ttl` | `mesh-formal` | Verified |
