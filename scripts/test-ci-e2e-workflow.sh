...
[ "$(grep -Fc 'libdbus-1-dev' .github/workflows/ci.yml)" -eq 3 ]
grep -F 'libdbus-1-dev' .github/workflows/release.yml >/dev/null
grep -F 'cargo test --manifest-path ../rust/Cargo.toml -p hashtree-network --test blob_router' .github/workflows/ci.yml >/dev/null
