...
grep -F -- "cargo test --locked -p hashtree-fuser --lib --test integration_tests -- --nocapture && cargo test --locked -p hashtree-cli --features fuse --test fuse_mount_smoke -- --nocapture" "${LOG_DIR}/docker.log" >/dev/null
