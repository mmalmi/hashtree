...
- Release `hashtree-fips-transport` 0.4.13 with `nvpn-fips-core` 0.4.74,
  `nvpn-fips-tcp` 0.2.1, and `nvpn-fips-tcp-endpoint` 0.2.10; advance the
  workspace to `nostr-pubsub-fips` 0.4.17. These updates improve connection
  startup and delivery through reconnects and key rotation without changing
  the protocol or the existing Rust dependency aliases. The adapter retains
  its `hashtree-core` 0.2.86 registry floor.
...
- Recover unavailable Git mirror objects from local loose or packed history
  after verifying their object IDs. Missing or corrupt local data still fails;
  multiline diagnostics no longer corrupt Git remote-helper responses.
...
- Root-brokered online source audits now open their exact source LMDB and
  ...
  use. Linux pinned opens no longer receive an ordinary namespace path, and
  source-path replacement cannot redirect the audit.
