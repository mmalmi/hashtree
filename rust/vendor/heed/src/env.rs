...
/// Caller-pinned identity of one LMDB file.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct PinnedLmdbFileIdentity {
    /// Device number reported by `stat`.
    pub device: u64,
    /// Inode number reported by `stat`.
    pub inode: u64,
}

/// Caller-pinned identities for both files in an LMDB environment.
///
/// The authority-establishing caller supplies these identities. Capturing
/// them inside `open` would leave a pathname-swap window between validation
/// and the LMDB open.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
#[cfg_attr(not(target_os = "linux"), allow(dead_code))]
pub struct PinnedLmdbIdentity {
    /// Authorized `data.mdb` identity.
    pub data: PinnedLmdbFileIdentity,
    /// Authorized `lock.mdb` identity.
    pub lock: PinnedLmdbFileIdentity,
}

impl PinnedLmdbIdentity {
    #[cfg(target_os = "linux")]
    fn validate_before_open(&self, path: &Path) -> io::Result<()> {
        if pinned_file_identity(&path.join("data.mdb"))? != self.data
            || pinned_file_identity(&path.join("lock.mdb"))? != self.lock
        {
            return Err(io::Error::other(
                "LMDB data/lock entry differs from the caller-pinned identity",
            ));
        }
        Ok(())
    }
    ...
    fn validate_before_open(&self, _path: &Path) -> io::Result<()> {
        Err(io::Error::new(
            io::ErrorKind::Unsupported,
            "pinned LMDB paths are supported only on Linux",
        ))
    }

    #[cfg(target_os = "linux")]
    unsafe fn validate_after_open(&self, env: *mut ffi::MDB_env, path: &Path) -> io::Result<()> {
        if pinned_file_identity(&path.join("data.mdb"))? != self.data
            || pinned_file_identity(&path.join("lock.mdb"))? != self.lock
        {
            return Err(io::Error::other(
                "pinned LMDB data/lock entry changed during open",
            ));
        }
        let mut data_fd = -1;
        let status = unsafe { ffi::mdb_env_get_fd(env, &mut data_fd) };
        if status != 0 {
            return Err(io::Error::from_raw_os_error(status));
        }
        if pinned_fd_identity(data_fd)? != self.data {
            return Err(io::Error::other(
                "LMDB opened a different data.mdb than the pinned entry",
            ));
        }
        let mut lock_fd = -1;
        let status = unsafe { ffi::mdb_env_get_lockfd(env, &mut lock_fd) };
        if status != 0 {
            return Err(io::Error::from_raw_os_error(status));
        }
        if pinned_fd_identity(lock_fd)? != self.lock {
            return Err(io::Error::other(
                "LMDB opened a different lock.mdb than the pinned entry",
            ));
        }
        Ok(())
    }
    ...
    #[cfg(not(target_os = "linux"))]
    unsafe fn validate_after_open(&self, _env: *mut ffi::MDB_env, _path: &Path) -> io::Result<()> {
        Err(io::Error::new(
            io::ErrorKind::Unsupported,
            "pinned LMDB paths are supported only on Linux",
        ))
...
#[cfg(target_os = "linux")]
fn pinned_file_identity(path: &Path) -> io::Result<PinnedLmdbFileIdentity> {
    ...
    if !metadata.is_file() {
        return Err(io::Error::new(
            ...
            "pinned LMDB entry is not a regular file",
        ));
    }
    Ok(PinnedLmdbFileIdentity {
        device: metadata.dev(),
        inode: metadata.ino(),
    })
}
...
#[cfg(target_os = "linux")]
fn pinned_fd_identity(fd: libc::c_int) -> io::Result<PinnedLmdbFileIdentity> {
...
impl EnvOpenOptions {
    ...
    pub unsafe fn open<P: AsRef<Path>>(&self, path: P) -> Result<Env> {
        ...
        unsafe { self.open_resolved_path(path, None) }
    }

    /// Opens an environment through an already-pinned absolute path without
    /// canonicalizing it first.
    ///
    /// This is intended for Linux `/proc/self/fd/N` directory handles. The
    /// ordinary [`Self::open`] canonicalization would turn such a handle back
    /// into a mutable pathname before LMDB opens its files, defeating the
    /// caller's retained directory descriptor.
    ///
    /// # Safety
    ///
    /// In addition to [`Self::open`]'s requirements, `path` must be an
    /// absolute, process-owned, retained path whose identity cannot be
    /// redirected for the lifetime of the returned environment.
    pub unsafe fn open_from_pinned_path<P: AsRef<Path>>(
        &self,
        path: P,
        pinned: PinnedLmdbIdentity,
    ) -> Result<Env> {
        let path = path.as_ref();
        if !path.is_absolute() {
            return Err(Error::Io(io::Error::new(
                io::ErrorKind::InvalidInput,
                "pinned LMDB path must be absolute",
            )));
        }
        pinned.validate_before_open(path)?;
        unsafe { self.open_resolved_path(path.to_path_buf(), Some(pinned)) }
    }

    unsafe fn open_resolved_path(
        &self,
        path: PathBuf,
            ...
            Entry::Vacant(entry) => {
                ...
                unsafe {
                    ...
                    match result {
                        Ok(()) => {
                            if let Some(pinned) = pinned {
                                if let Err(error) = pinned.validate_after_open(env, path) {
