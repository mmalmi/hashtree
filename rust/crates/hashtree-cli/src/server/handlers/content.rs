...
pub(super) async fn query_webrtc_peers(
    ...
    #[cfg(feature = "p2p")]
    if let Some((data, peer_id)) = query_known_http_peers(webrtc_state, hash_hex).await {
        tracing::info!(
            "Got {} bytes from known peer {} for hash {}",
            data.len(),
            peer_id,
            &hash_hex[..16.min(hash_hex.len())]
        );
...
#[cfg(feature = "p2p")]
async fn query_known_http_peers(
    webrtc_state: &Arc<WebRTCState>,
    hash_hex: &str,
) -> Option<(Vec<u8>, String)> {
    ...
    for peer in webrtc_state.ordered_known_peers().await {
        for address in peer.addresses {
            ...
            pending.push(async move {
                let url = format!("{address}/{hash_hex}");
                let sent_len = url.len() as u64;
                webrtc_state
                    .record_direct_peer_request(&peer_id, sent_len)
                    .await;
                let started = tokio::time::Instant::now();
                (peer_id, started, client.get(url).send().await)
            });
        }
    }

    while let Some((peer_id, started, result)) = pending.next().await {
        match result {
            Ok(resp) if resp.status().is_success() => match resp.bytes().await {
                Ok(bytes) => {
                    let mut hasher = Sha256::new();
                    hasher.update(&bytes);
                    if hex::encode(hasher.finalize()) == hash_hex {
                        let data = bytes.to_vec();
                        webrtc_state
                            .record_direct_peer_success(
                                &peer_id,
                    ...
                    webrtc_state.record_direct_peer_failure(&peer_id).await;
                }
                Err(_) => {
                    webrtc_state.record_direct_peer_failure(&peer_id).await;
                }
            ...
            Ok(resp) if resp.status() == StatusCode::NOT_FOUND => {
                webrtc_state.record_direct_peer_miss(&peer_id).await;
            }
            ...
            Err(err) if err.is_timeout() => {
                webrtc_state.record_direct_peer_timeout(&peer_id).await;
            }
            Err(_) => {
                webrtc_state.record_direct_peer_failure(&peer_id).await;
            }
